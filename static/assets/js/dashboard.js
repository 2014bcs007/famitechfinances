...
if (graphs) {
  ...
  Object.values(graphs).forEach(graph => {
    if (graph && graph.key) {
      var data=[],data2=[],labels=[],colors=[],datasets=[];
      if(graph.data&&graph.labels){
        graph.data.forEach((row,index)=>{
          labels.push(row[graph.labels['label']]);
          if (Array.isArray(row[graph.labels['value']])){
            // data.push(row[graph.labels['value']][0]);
            // data2.push(row[graph.labels['value']][1]);
        ...
        if(!graph.multiple){
          ...
          graph.data.forEach((row,index)=>{
            if (Array.isArray(row[graph.labels['value']])){
              row[graph.labels['value']].forEach((data_,i)=>{
                // dSet[index][data_.label_id]=data_.value
                // console.log(data_.label,data_.value);
              });
        ...
        //   datasets.push({
        //     data: data2,
        //     backgroundColor: returnColorCode(4),
        //     borderColor:returnColorCode(4),
        //     fill:graph.fill,
        //     label:"Label2"
        //   },)
        // }
